import { ProductList, ProductTypes } from "./Products.types";

export const title = "Наша продукция";
export const description =
  "Выберите из нашего огромного каталога интересующий Вас вариант, и оставьте заявку на его приобретение. Мы обязательно свяжемся с Вами!";
export const showAllBtnText = "Показать все варианты";
export const collapseBtnText = "Свернуть";
export const emptyPlaceHolderText = "К сожалению, в данный момент нет товаров, соответствующих данным критериям."

export const products: ProductList = {
  [ProductTypes.Matrases]: [
    {
      title: "Турин-1",
      content: [
        { id: "1", element: "Кокосовая койра", value: "2 см" },
        { id: "2", element: "Войлок термоскрепленный", value: "0,5 см" },
        { id: "3", element: "Кокосовая койра", value: "3 см" },
        { id: "4", element: "Высота матраца", value: "16 см" },
        { id: "5", element: "Для людей весом", value: "до 120 кг" },
      ],
      imagePath: "/assets/product/turin-1.webp",
      imageDescription: "Изображение структуры матраса Турин-1",
      sizes: [
        {
          price: 15000,
          size: "220х220",
        },
        {
          price: 13500,
          size: "200х200",
        },
        {
          price: 10000,
          size: "180х200",
        },
      ],
      link: "#contacts",
    },
    {
      title: "Турин-2",
      content: [
        { id: "1", element: "Кокосовая койра", value: "2 см" },
        { id: "2", element: "Войлок термоскрепленный", value: "0,5 см" },
        { id: "3", element: "Кокосовая койра", value: "3 см" },
        { id: "4", element: "Высота матраца", value: "16 см" },
        { id: "5", element: "Для людей весом", value: "до 120 кг" },
      ],
      imagePath: "/assets/product/turin-2.webp",
      imageDescription: "Изображение структуры матраса Турин-2",
      sizes: [
        {
          price: 12000,
          size: "220х220",
        },
        {
          price: 10500,
          size: "200х200",
        },
        {
          price: 9000,
          size: "180х200",
        },
      ],
      link: "#contacts",
    },
    {
      title: "Турин-3",
      content: [
        { id: "1", element: "Кокосовая койра", value: "2 см" },
        { id: "2", element: "Войлок термоскрепленный", value: "0,5 см" },
        { id: "3", element: "Кокосовая койра", value: "3 см" },
        { id: "4", element: "Высота матраца", value: "16 см" },
        { id: "5", element: "Для людей весом", value: "до 120 кг" },
      ],
      imagePath: "/assets/product/turin-3.webp",
      imageDescription: "Изображение структуры матраса Турин-3",
      sizes: [
        {
          price: 11000,
          size: "220х220",
        },
        {
          price: 9500,
          size: "200х200",
        },
        {
          price: 8000,
          size: "180х200",
        },
      ],
      link: "#contacts",
    },
    {
      title: "Флоренция-1",
      content: [
        { id: "1", element: "Кокосовая койра", value: "2 см" },
        { id: "2", element: "Войлок термоскрепленный", value: "0,5 см" },
        { id: "3", element: "Кокосовая койра", value: "3 см" },
        { id: "4", element: "Высота матраца", value: "16 см" },
        { id: "5", element: "Для людей весом", value: "до 120 кг" },
      ],
      imagePath: "/assets/product/florencia-1.webp",
      imageDescription: "Изображение структуры матраса Флоренция-1",
      sizes: [
        {
          price: 15000,
          size: "220х220",
        },
        {
          price: 13500,
          size: "200х200",
        },
        {
          price: 10000,
          size: "180х200",
        },
      ],
      link: "#contacts",
    },
    {
      title: "Флоренция-2",
      content: [
        { id: "1", element: "Кокосовая койра", value: "2 см" },
        { id: "2", element: "Войлок термоскрепленный", value: "0,5 см" },
        { id: "3", element: "Кокосовая койра", value: "3 см" },
        { id: "4", element: "Высота матраца", value: "16 см" },
        { id: "5", element: "Для людей весом", value: "до 120 кг" },
      ],
      imagePath: "/assets/product/turin-1.webp",
      imageDescription: "Изображение структуры матраса Флоренция-2",
      sizes: [
        {
          price: 12000,
          size: "220х220",
        },
        {
          price: 10500,
          size: "200х200",
        },
        {
          price: 9000,
          size: "180х200",
        },
      ],
      link: "#contacts",
    },
    {
      title: "Флоренция-3",
      content: [
        { id: "1", element: "Кокосовая койра", value: "2 см" },
        { id: "2", element: "Войлок термоскрепленный", value: "0,5 см" },
        { id: "3", element: "Кокосовая койра", value: "3 см" },
        { id: "4", element: "Высота матраца", value: "16 см" },
        { id: "5", element: "Для людей весом", value: "до 120 кг" },
      ],
      imagePath: "/assets/product/turin-2.webp",
      imageDescription: "Изображение структуры матраса Флоренция-3",
      sizes: [
        {
          price: 11000,
          size: "220х220",
        },
        {
          price: 9500,
          size: "200х200",
        },
        {
          price: 8000,
          size: "180х200",
        },
      ],
      link: "#contacts",
    },
    {
      title: "Флоренция-4",
      content: [
        { id: "1", element: "Кокосовая койра", value: "2 см" },
        { id: "2", element: "Войлок термоскрепленный", value: "0,5 см" },
        { id: "3", element: "Кокосовая койра", value: "3 см" },
        { id: "4", element: "Высота матраца", value: "16 см" },
        { id: "5", element: "Для людей весом", value: "до 120 кг" },
      ],
      imagePath: "/assets/product/turin-3.webp",
      imageDescription: "Изображение структуры матраса Флоренция-4",
      sizes: [
        {
          price: 11000,
          size: "220х220",
        },
        {
          price: 9500,
          size: "200х200",
        },
        {
          price: 8000,
          size: "180х200",
        },
      ],
      link: "#contacts",
    },
    {
      title: "Неаполь-1",
      content: [
        { id: "1", element: "Кокосовая койра", value: "2 см" },
        { id: "2", element: "Войлок термоскрепленный", value: "0,5 см" },
        { id: "3", element: "Кокосовая койра", value: "3 см" },
        { id: "4", element: "Высота матраца", value: "16 см" },
        { id: "5", element: "Для людей весом", value: "до 120 кг" },
      ],
      imagePath: "/assets/product/florencia-1.webp",
      imageDescription: "Изображение структуры матраса Неаполь-1",
      sizes: [
        {
          price: 11000,
          size: "220х220",
        },
        {
          price: 9500,
          size: "200х200",
        },
        {
          price: 8000,
          size: "180х200",
        },
      ],
      link: "#contacts",
    },
    {
      title: "Неаполь-2",
      content: [
        { id: "1", element: "Кокосовая койра", value: "2 см" },
        { id: "2", element: "Войлок термоскрепленный", value: "0,5 см" },
        { id: "3", element: "Кокосовая койра", value: "3 см" },
        { id: "4", element: "Высота матраца", value: "16 см" },
        { id: "5", element: "Для людей весом", value: "до 120 кг" },
      ],
      imagePath: "/assets/product/florencia-1.webp",
      imageDescription: "Изображение структуры матраса Неаполь-2",
      sizes: [
        {
          price: 11000,
          size: "220х220",
        },
        {
          price: 9500,
          size: "200х200",
        },
        {
          price: 8000,
          size: "180х200",
        },
      ],
      link: "#contacts",
    },
    {
      title: "Неаполь-3",
      content: [
        { id: "1", element: "Кокосовая койра", value: "2 см" },
        { id: "2", element: "Войлок термоскрепленный", value: "0,5 см" },
        { id: "3", element: "Кокосовая койра", value: "3 см" },
        { id: "4", element: "Высота матраца", value: "16 см" },
        { id: "5", element: "Для людей весом", value: "до 120 кг" },
      ],
      imagePath: "/assets/product/florencia-1.webp",
      imageDescription: "Изображение структуры матраса Неаполь-3",
      sizes: [
        {
          price: 11000,
          size: "220х220",
        },
        {
          price: 9500,
          size: "200х200",
        },
        {
          price: 8000,
          size: "180х200",
        },
      ],
      link: "#contacts",
    },
    {
      title: "Неаполь-4",
      content: [
        { id: "1", element: "Кокосовая койра", value: "2 см" },
        { id: "2", element: "Войлок термоскрепленный", value: "0,5 см" },
        { id: "3", element: "Кокосовая койра", value: "3 см" },
        { id: "4", element: "Высота матраца", value: "16 см" },
        { id: "5", element: "Для людей весом", value: "до 120 кг" },
      ],
      imagePath: "/assets/product/florencia-1.webp",
      imageDescription: "Изображение структуры матраса Неаполь-4",
      sizes: [
        {
          price: 11000,
          size: "220х220",
        },
        {
          price: 9500,
          size: "200х200",
        },
        {
          price: 8000,
          size: "180х200",
        },
      ],
      link: "#contacts",
    },
  ],
  [ProductTypes.Covers]: [],
  [ProductTypes.ChildrenMatrases]: [],
};
